...
(defn page [{:keys [] :as req}]  
  (html5
   ...
   [:head
    ...
    [:title "Components"]
    #_[:link {:rel "stylesheet" :href "/node_modules/bootstrap/dist/css/bootstrap.min.css"} ]
    #_[:link {:rel "stylesheet" :href "/node_modules/jquery-ui/jquery-ui.min.css"} ]
    #_[:link {:rel "stylesheet" :href "/node_modules/fullcalendar/dist/fullcalendar.min.css"} ]
   ...
   [:body
    ...
    [:script {:src "/node_modules/fullcalendar/dist/locale-all.js"}]
    [:main
     (if (= (:dev env) "true")
       {:class "dev"})
     "Loading..."]]))
